...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        int res = fscanf(file, " %u %u %u", &k_year, &k_month, &k_day); //indexo los primeros tres numeros (año,mes,dia)
        if (res != 3) {
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
        Weather weather = weather_from_file(file); //los demas numeros se los delego a una funcion weater tomandos como una tupla
                   
        array[k_year - FST_YEAR][k_month - 1u][k_day - FST_DAY] = weather;           //inicializar las posiciones //igual a weather estructura
